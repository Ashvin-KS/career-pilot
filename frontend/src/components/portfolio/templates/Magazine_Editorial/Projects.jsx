...
const KEYFRAME_CSS = `
  ...
  .ag-card-hover:hover {
    transform: translateY(0px) rotate(0deg) scale(1.038) !important;
...
/* ─── Static star / dot positions (stable, no random on re-render) ────────── */
...
const CARD_CONFIG = [
  { issue: 'No. 01', category: 'COVER STORY', accent: '#DC2626', accentLight: 'rgba(220,38,38,0.12)', drift: 'drift-0', dur: '7.2s', delay: '0s',   zBase: 1 },
  { issue: 'No. 02', category: 'FEATURE',     accent: '#B45309', accentLight: 'rgba(180,83,9,0.12)',  drift: 'drift-1', dur: '8.8s', delay: '1.1s',  zBase: 2 },
  { issue: 'No. 03', category: 'DISPATCH',    accent: '#166534', accentLight: 'rgba(22,101,52,0.12)', drift: 'drift-2', dur: '6.5s', delay: '0.4s',  zBase: 1 },
  { issue: 'No. 04', category: 'EDITORIAL',   accent: '#6B21A8', accentLight: 'rgba(107,33,168,0.12)',drift: 'drift-3', dur: '9.3s', delay: '1.8s',  zBase: 2 },
  { issue: 'No. 05', category: 'SPOTLIGHT',   accent: '#0C4A6E', accentLight: 'rgba(12,74,110,0.12)', drift: 'drift-4', dur: '7.9s', delay: '0.7s',  zBase: 1 },
  { issue: 'No. 06', category: 'LONG READ',   accent: '#9D174D', accentLight: 'rgba(157,23,77,0.12)', drift: 'drift-5', dur: '8.1s', delay: '2.2s',  zBase: 2 },
  { issue: 'No. 07', category: 'OPEN SOURCE', accent: '#0E7490', accentLight: 'rgba(14,116,144,0.12)',drift: 'drift-0', dur: '8.4s', delay: '0.9s',  zBase: 1 },
  { issue: 'No. 08', category: 'ANALYTICS',   accent: '#A16207', accentLight: 'rgba(161,98,7,0.12)',  drift: 'drift-1', dur: '7.6s', delay: '1.5s',  zBase: 2 },
  { issue: 'No. 09', category: 'DESIGN SYS',  accent: '#4338CA', accentLight: 'rgba(67,56,202,0.12)', drift: 'drift-2', dur: '9.1s', delay: '3.0s',  zBase: 1 },
  { issue: 'No. 10', category: 'DEV TOOLS',   accent: '#065F46', accentLight: 'rgba(6,95,70,0.12)',   drift: 'drift-3', dur: '6.8s', delay: '0.2s',  zBase: 2 },
];

/* ─── Decorative floating rule lines ───────────────────────────────────────── */
const RULES = [
...
/* ─── Drop-cap letters drifting behind cards ────────────────────────────────── */
const DROP_CAPS = [
...
export default function Projects() {
  ...
  /* Pad real projects to 10 with sensible fallbacks */
  const EXTRA = [
    {
      title: 'DevFlow CLI',
    ...
    },
    {
      ...
      liveUrl: '#', githubUrl: '#',
      image: 'https://images.unsplash.com/photo-1633613286991-611fe299c4be?w=800&h=450&fit=crop',
    },
    {
      title: 'Tokens DS',
    ...
    },
    {
      ...
      liveUrl: '#', githubUrl: '#',
      image: 'https://images.unsplash.com/photo-1607799279861-4dd421887fb3?w=800&h=450&fit=crop',
    },
  ];
  ...
  return (
    ...
    >
      ...
      <button
        ...
        onMouseEnter={(e) => { e.currentTarget.style.boxShadow = isDark ? '0 4px 20px rgba(0,0,0,0.7)' : '0 4px 16px rgba(0,0,0,0.14)'; }}
        onMouseLeave={(e) => { e.currentTarget.style.boxShadow = isDark ? '0 2px 12px rgba(0,0,0,0.5)' : '0 2px 10px rgba(0,0,0,0.08)'; }}
      >
        {isDark ? (
      ...
      {/* ── Star-field background (CSS-only, no images) ─────────────────── */}
      <div
        style={{
          position: 'absolute',
          inset: 0,
      ...
      >
        ...
        <div
          style={{
            ...
            inset: 0,
            backgroundImage:
              'radial-gradient(rgba(240,234,216,0.06) 1px, transparent 1px)',
            backgroundSize: '40px 40px',
          }}
        />
      </div>

      {/* ── Floating geometric rule lines ───────────────────────────────── */}
      ...
      {/* ── Large drop-cap drifting letters ─────────────────────────────── */}
        ...
        {DROP_CAPS.map((d, i) => (
          <span
            ...
            style={{
              ...
              lineHeight: 1,
              userSelect: 'none',
              animation: `drop-cap-drift ${d.dur} ${d.delay} ease-in-out infinite`,
            }}
          >
            {d.char}
          </span>
      ...
      >
        ...
        <header
          style={{
            textAlign: 'center',
            marginBottom: '4rem',
            animation: mounted ? 'masthead-slide 0.9s ease both' : 'none',
          }}
        >
          {/* Eyebrow label */}
          <div
          ...
          >
            <div style={{ flex: 1, maxWidth: '200px', height: '1px', background: `linear-gradient(90deg, transparent, ${T.text})`, opacity: 0.25, transition: 'background 0.45s ease' }} />
            <span
              style={{
                fontSize: '0.6rem',
                letterSpacing: '0.3em',
            ...
            <div style={{ flex: 1, maxWidth: '200px', height: '1px', background: `linear-gradient(90deg, ${T.text}, transparent)`, opacity: 0.25, transition: 'background 0.45s ease' }} />
          </div>
        ...
        {/* ── Card Grid ─────────────────────────────────────────────────── */}
        {/*
          Desktop: 3-col asymmetric grid with varying row-spans.
          Tablet:  2-col grid.
          Mobile:  1-col, no tilts.
        */}
        <div
          style={{
            display: 'grid',
        ...
        >
          <div style={{ flex: 1, height: '1px', background: T.footerRule, opacity: 0.3, transition: 'background 0.45s ease' }} />
          <Rocket size={14} color={T.footerRule} style={{ opacity: 0.3, transition: 'color 0.45s ease' }} />
          <div style={{ flex: 1, height: '1px', background: T.footerRule, opacity: 0.3, transition: 'background 0.45s ease' }} />
        </div>
        <p
          style={{
...
function ProjectCard({ project, cfg, index, isHovered, mounted, animDelay, isDark, T, onMouseEnter, onMouseLeave }) {
  ...
  const cardStyle = {
    ...
    /* Hover snap */
    transform: isHovered
      ? 'translateY(-6px) rotate(0deg) scale(1.03)'
      : undefined,
    boxShadow: isHovered
      ? T.shadowHover(cfg.accent)
      : T.shadowCard,
  ...
  return (
    ...
    >
      ...
      {/* Bottom-right decorative issue mark */}
      <div
        style={{
          position: 'absolute',
          bottom: '0.6rem',
